...
const LeagueOfLegendsStats = ({ username, tag }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://gamer-insights.azurewebsites.net/api/leaguegetpuuid?code=S-qc4dIw6awRjX4zf2-8Fq5-F3sZjm2R1IicSv7YRN6cAzFusHAL1A%3D%3D&username=${username}&tag=${tag}`);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://gamer-insights.azurewebsites.net/api/getchampionmastery?code=kr0MFXnnRCrd4MUdqO2nv4IoaFsMotgiAKfVj2Hb-BFeAzFuSjNExg%3D%3D&puuid=${puuid}`);
    ...
    useEffect(() => {
        const fetchChampionData = async () => {
            try {
                const response = await axios.get("https://ddragon.leagueoflegends.com/cdn/14.3.1/data/en_US/champion.json");
